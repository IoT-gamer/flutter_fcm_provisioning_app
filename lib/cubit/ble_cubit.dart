...
class BleCubit extends Cubit<BleState> {
  ...
  final Guid _fcmUrlCharUuid = Guid("5a67d678-6361-4f32-8396-54c6926c8fa6");
  final Guid _fcmTokenCharUuid = Guid("5a67d678-6361-4f32-8396-54c6926c8fa7");

  BluetoothCharacteristic? _ssidChar;
  BluetoothCharacteristic? _passwordChar;
  BluetoothCharacteristic? _commandChar;
  BluetoothCharacteristic? _pairingStatusChar;
  BluetoothCharacteristic? _fcmUrlChar;
  BluetoothCharacteristic? _fcmTokenChar;
    ...
    for (BluetoothService service in services) {
      ...
      if (service.uuid == _serviceUuid) {
        for (BluetoothCharacteristic c in service.characteristics) {
          ...
          if (c.uuid == _fcmUrlCharUuid) _fcmUrlChar = c;
          if (c.uuid == _fcmTokenCharUuid) _fcmTokenChar = c;
        }
  ...
  Future<void> sendCredentials(
    ...
    if (_ssidChar == null ||
      ...
      emit(
        state.copyWith(
          errorMessage: "All required characteristics not found. Cannot send.",
        ),
      );
      return;
    }
    ...
    try {
      // Write all credential characteristics first
      ...
      await _fcmUrlChar!.write(utf8.encode(fcmUrl), withoutResponse: false);
      await _fcmTokenChar!.write(utf8.encode(fcmToken), withoutResponse: false);
      print("SSID, Password, FCM URL and Token written.");

      // *** START: MODIFIED LOGIC ***

      // First, if requested, send the command to SAVE the network and all credentials.
      if (saveNetwork) {
        ...
        // Give the peripheral a moment to process the save operation.
        await Future.delayed(const Duration(milliseconds: 500));
      }

      // Second, send the command to CONNECT to the WiFi network.
  ...
  void _resetConnectionState() {
    ...
    _commandChar = null;
    // removeBond();
    _fcmUrlChar = null; // reset
    _fcmTokenChar = null; // reset
    if (!isClosed) {
      emit(
        state.copyWith(
