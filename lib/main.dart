...
import 'package:flutter_fcm_provisioning_app/constants.dart' as constants;
import 'package:flutter_fcm_provisioning_app/firebase_options.dart';
...
class _BleControlScreenState extends State<BleControlScreen> {
  ...
  final _fcmUrlController = TextEditingController();
  ...
  void dispose() {
    ...
    _fcmUrlController.dispose();
    _fcmTokenController.dispose();
    super.dispose();
  }

  // New function to get FCM details
  Future<void> _getFcmDetails() async {
    // Cloud Function URL
    const fcmUrl = constants.fcmFunctionUrl;
    _fcmUrlController.text = fcmUrl;

    final fcmToken = await FirebaseMessaging.instance.getToken();
    if (fcmToken != null) {
      _fcmTokenController.text = fcmToken;
  ...
  Widget _buildConnectionControl(BuildContext context, BleState state) {
    String name = state.selectedDevice!.platformName.isNotEmpty
  ...
  Widget _buildProvisioningSection(BuildContext context, BleState state) {
    ...
    return Column(
      ...
      children: [
        ...
        Text('FCM Details', style: Theme.of(context).textTheme.titleMedium),
        const SizedBox(height: 8),

        // FCM URL TextField
        TextField(
          controller: _fcmUrlController,
          enabled: canProvision && !state.isLoading,
          decoration: const InputDecoration(
            labelText: 'FCM Cloud Function URL',
            border: OutlineInputBorder(),
          ),
        ),
        const SizedBox(height: 12),
        ...
        TextField(
          ...
          readOnly: true, // Token should be populated by the button
        ),
        const SizedBox(height: 8),
        Center(
          child: ElevatedButton(
            onPressed: (state.isLoading || !canProvision)
                ? null
                : _getFcmDetails,
        ...
        Padding(
          ...
          child: CheckboxListTile(
            ...
            value: _saveNetwork,
            onChanged: (canProvision && !state.isLoading)
                ? (bool? newValue) {
                    if (newValue != null) {
                      setState(() {
          ...
          child: ElevatedButton(
            onPressed: (state.isLoading || !canProvision)
                ...
                : () {
                    ...
                    final fcmUrl = _fcmUrlController.text;
                    final fcmToken = _fcmTokenController.text;
                    if (ssid.isNotEmpty) {
            ...
            child: const Text('Send All Credentials & Connect'),
          ),
        ),
